...
#include "../../teaForGodAndroid.NativeActivity/android_native_app_glue.h"
...
#include <openxr_oculus_helpers.h>
...
static ovrUniform ProgramUniforms[] = {
    {ovrUniform::MODEL_MATRIX, ovrUniform::MATRIX4X4, "modelMatrix"},
    {ovrUniform::VIEW_PROJ_MATRIX, ovrUniform::MATRIX4X4, "viewProjectionMatrix"},
};
...
ovrProgram_Create(ovrProgram* program, const char* vertexSource, const char* fragmentSource) {
    ...
    for (size_t i = 0; i < sizeof(ProgramUniforms) / sizeof(ProgramUniforms[0]); i++) {
        ...
        if (ProgramUniforms[i].type == ovrUniform::BUFFER) {
...
static bool ovrFramebuffer_Create(
    ...
    swapChainCreateInfo.format = GL_RGBA8;// ,//GL_RGBA8,//GL_SRGB8_ALPHA8, // !@# doesn't matter what goes here
    swapChainCreateInfo.sampleCount = multisamples > 1? multisamples : 1;
...
ovrScene_Create(AAssetManager* amgr, XrInstance instance, XrSession session, ovrScene* scene) {
    ...
    // Simple checkerboard pattern.
    {
        ...
        swapChainCreateInfo.usageFlags =
            XR_SWAPCHAIN_USAGE_SAMPLED_BIT | XR_SWAPCHAIN_USAGE_COLOR_ATTACHMENT_BIT;
        swapChainCreateInfo.format = GL_SRGB8_ALPHA8;// GL_RGBA8;// GL_SRGB8_ALPHA8; // !@# here srgb gives correct results and rgb gives washed out colours, just as expected
        swapChainCreateInfo.sampleCount = 1;
        swapChainCreateInfo.width = width;
        swapChainCreateInfo.height = height;
        swapChainCreateInfo.faceCount = 1;
...

        {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    float aFrac = 1.0f;// (float)((x / 32) % 2);
					texData[y * width + x] = ((uint32_t)(255 * aFrac)) << 24 | ((uint32_t)(255 * bFrac)) << 16 | ((uint32_t)(255 * gFrac)) << 8 | ((uint32_t)(255 * rFrac));
                }
            }
        }
        /*
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                texData[y * width + x] = 0xFF000000  | ((uint32_t)(255 * greenfrac)) << 16 | ((uint32_t)(255 * greenfrac)) << 8 | ((uint32_t)(255 * greenfrac));
            }
        }
        */
...
static void ovrRenderer_Create(
    ...
    for (int eye = 0; eye < ovrMaxNumEyes; eye++) {
        ovrFramebuffer_Create(
            ...
            GL_SRGB8_ALPHA8,//GL_RGBA8,//GL_SRGB8_ALPHA8, // !@# doesn't matter what goes here
...
static void ovrRenderer_RenderFrame(
    ...
    for (int eye = 0; eye < ovrMaxNumEyes; eye++) {
        ...
        glUniformMatrix4fv(
            scene->Program.UniformLocation[ovrUniform::MODEL_MATRIX], 1, GL_FALSE, &modelMatrix.m[0]);
        ...
        glUniformMatrix4fv(
            scene->Program.UniformLocation[ovrUniform::VIEW_PROJ_MATRIX], 1, GL_FALSE, &viewProjMatrix.m[0]);
        ...
        float gamma = 1.0f;
        GL(glClearColor(powf(1.0f, gamma), powf(0.7f, gamma), powf(0.2f, gamma), clearAlpha));
        ...
        for (int i = 0; i < 4; i++) {
            ...
            glUniformMatrix4fv(
                scene->Program.UniformLocation[ovrUniform::MODEL_MATRIX], 1, GL_FALSE, &model.m[0]);
...
static void ovrRenderer_SetFoveation(
    ...
    if (pfnCreateFoveationProfileFB &&
        pfnDestroyFoveationProfileFB &&
...
void test_sample_android_main_oculus(struct android_app* app) {
    ...
    while (app->destroyRequested == 0) {
        ...
        for (int eye = 0; eye < ovrMaxNumEyes; eye++) {
            ...
            // todo - fix it for new openxr libs
            /*
        ...
        */
        ...
        if (shouldRenderWorldLayer) {
            ...
            projection_layer.layerFlags = 0;//XR_COMPOSITION_LAYER_BLEND_TEXTURE_SOURCE_ALPHA_BIT;
