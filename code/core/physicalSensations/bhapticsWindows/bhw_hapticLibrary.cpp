...
#include "bhw_model.h"
#include "bhw_util.h"
#include "bhw_commonutils.h"
#include <iostream>
#include "bhw_projectModel.h"
...
const char* bhapticsLibrary::GetExePath() {
    if (isInit)
    {
...
//char* getExePath
bool bhapticsLibrary::TryGetExePath(char* buf, int maxSize, int& buf_size)
{
    if (isInit)
    {
        buf_size = (int)exeFilePath.size();
        strcpy_s(buf, maxSize - 1, exeFilePath.c_str());
        return true;
    }
    isInit = true;
#if defined _WIN32 || defined _WIN64 ||  defined(__WIN32)
...
void bhapticsLibrary::SubmitPathArray(const char *key, bhaptics::PositionType Pos, point *points, size_t length,
                     int durationMillis)
{
    ...
    bhapticsLibrary::SubmitPath(key, Pos, motorbyts, durationMillis);
}

void bhapticsLibrary::SubmitByteArray(const char* key, bhaptics::PositionType Pos, unsigned char* buf, size_t length, int durationMillis)
{
    ...
    bhapticsLibrary::Submit(key, Pos, motorbyts, durationMillis);
}

void bhapticsLibrary::ChangeUrl(const char* url)
{
    bhaptics::HapticPlayer::instance()->changeUrl(url);
}

void bhapticsLibrary::InitialiseSync(const char* appId, const char* appName)
{
    bhaptics::HapticPlayer::instance()->registerConnection(appId, appName);
}

void bhapticsLibrary::Initialise(const char* appId, const char* appName) {
    std::thread thread (bhapticsLibrary::InitialiseSync, appId, appName);
    thread.detach();
}

void bhapticsLibrary::Destroy()
{
    bhaptics::HapticPlayer::instance()->destroy();
}

void bhapticsLibrary::RegisterFeedback(const char* key, const char* projectJson)
{
    bhaptics::HapticPlayer::instance()->registerFeedbackFromString(key, projectJson);
}

void bhapticsLibrary::RegisterFeedbackFromTactFile(const char* key, const char* tactFileStr)
{
...
void bhapticsLibrary::RegisterFeedbackFromTactFileReflected(const char* key, const char* tactFileStr)
{
    auto hapticFile = Util::parseFromTactFileString(tactFileStr);
...
void bhapticsLibrary::SubmitRegistered(const char* key)
{
    bhaptics::HapticPlayer::instance()->submitRegistered(key);
}

void bhapticsLibrary::SubmitRegisteredAlt(const char* Key, const char* AltKey, bhaptics::ScaleOption ScaleOpt, bhaptics::RotationOption RotOption)
{
    bhaptics::HapticPlayer::instance()->submitRegistered(Key, AltKey, ScaleOpt, RotOption);
}

void bhapticsLibrary::SubmitRegisteredWithOption(const char* Key, const char* AltKey, float intensity, float duration, float offsetAngleX, float offsetAngleY)
{
...
void bhapticsLibrary::Submit(const char* Key, bhaptics::PositionType Pos, std::vector<uint8_t>& MotorBytes, int DurationMillis)
{
    bhaptics::HapticPlayer::instance()->submit(Key, Pos, MotorBytes, DurationMillis);
}
void bhapticsLibrary::SubmitRegisteredStartMillis(const char* key, int startMillis)
{
    bhaptics::HapticPlayer::instance()->submitRegistered(key, startMillis);
}

void bhapticsLibrary::SubmitDot(const char* Key, bhaptics::PositionType Pos, std::vector<bhaptics::DotPoint>& Points, int DurationMillis)
{
    bhaptics::HapticPlayer::instance()->submit(Key, Pos, Points, DurationMillis);
}

void bhapticsLibrary::SubmitPath(const char* Key, bhaptics::PositionType Pos, std::vector<bhaptics::PathPoint>& Points, int DurationMillis)
{
    bhaptics::HapticPlayer::instance()->submit(Key, Pos, Points, DurationMillis);
}

bool bhapticsLibrary::IsFeedbackRegistered(const char* key)
{
    return bhaptics::HapticPlayer::instance()->isFeedbackRegistered(key);
}

bool bhapticsLibrary::IsPlaying()
{
    return bhaptics::HapticPlayer::instance()->isPlaying();
}

bool bhapticsLibrary::IsPlayingKey(const char* Key)
{
    return bhaptics::HapticPlayer::instance()->isPlaying(Key);
}

void bhapticsLibrary::TurnOff()
{
    bhaptics::HapticPlayer::instance()->turnOff();
}

void bhapticsLibrary::TurnOffKey(const char* Key)
{
    bhaptics::HapticPlayer::instance()->turnOff(Key);
}

void bhapticsLibrary::EnableFeedback()
{
    bhaptics::HapticPlayer::instance()->enableFeedback();
}

void bhapticsLibrary::DisableFeedback()
{
    bhaptics::HapticPlayer::instance()->disableFeedback();
}

void bhapticsLibrary::ToggleFeedback()
{
    bhaptics::HapticPlayer::instance()->toggleFeedback();
}

bool bhapticsLibrary::IsDevicePlaying(bhaptics::PositionType Pos)
{
    return bhaptics::HapticPlayer::instance()->isDevicePlaying(Pos);
}

bool bhapticsLibrary::TryGetResponseForPosition(bhaptics::PositionType Pos, status& s)
{
