...
REGISTER_FOR_FAST_CAST(BhapticsJava);

void BhapticsJava::splash_logo()
{
	Splash::Logos::add(TXT("bhaptics"), SPLASH_LOGO_DEVICE);
}

Name const& BhapticsJava::system_id()
{
	return NAME(bhaptics);
}

void BhapticsJava::start()
{
	terminate();
...
void BhapticsJava::async_init()
{
	if (initialising.acquire_if_not_locked())
	{
		bhapticsModule = new bhapticsJava::BhapticsModule();

		isOk = true;
	}
}

void BhapticsJava::internal_start_sensation(REF_ SingleSensation & _sensation)
{
#ifdef LOG_BHAPTICS
	...
	ArrayStatic<an_bhaptics::SensationToTrigger, 4> sensations; SET_EXTRA_DEBUG_INFO(sensations, TXT("BhapticsJava::internal_start_sensation(single).sensations"));
	...
	for_every(s, sensations)
	{
			...
			bhapticsModule->get_available_devices().has(s->position))
		{
			if (s->position == an_bhaptics::PositionType::Vest)
			{
				...
				bhapticsModule->submit_sensation(s->sensationId, offsetYaw, offsetZ);
			}
			else
			{
...
void BhapticsJava::internal_start_sensation(REF_ OngoingSensation & _sensation)
{
#ifdef LOG_BHAPTICS
	...
	ArrayStatic<an_bhaptics::SensationToTrigger, 4> sensations; SET_EXTRA_DEBUG_INFO(sensations, TXT("BhapticsJava::internal_start_sensation(ongoing).sensations"));
	an_bhaptics::Library::get_sensation_ids_for(_sensation, bhapticsModule->get_available_devices(), sensations);
	for_every(s, sensations)
	{
#ifdef LOG_BHAPTICS
...
void BhapticsJava::internal_sustain_sensation(Sensation::ID _id)
{
	for_every(os, ongoingSensations)
	{
		if (os->id == _id)
		{
			if (!bhapticsModule->is_sensation_active(os->sensationId))
			{
				...
				if (os->vest)
				{
					bhapticsModule->submit_sensation(os->sensationId, os->offsetYaw, os->offsetZ);
				}
				else
				{
...
void BhapticsJava::internal_stop_sensation(Sensation::ID _id)
{
...
bool BhapticsJava::internal_stop_all_sensations()
{
