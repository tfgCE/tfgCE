...
DefinedFlags::DefinedFlag const * DefinedFlags::s_ikTrace = nullptr;
DefinedFlags::DefinedFlag const * DefinedFlags::s_ikTraceReject = nullptr;
DefinedFlags::DefinedFlag const * DefinedFlags::s_presenceTrace = nullptr;
DefinedFlags::DefinedFlag const * DefinedFlags::s_presenceTraceReject = nullptr;
DefinedFlags::DefinedFlag const * DefinedFlags::s_visionTrace = nullptr;
...
bool DefinedFlags::check_and_cache()
{
	...
	HAS_MASK(TXT("actorCollisionFlags"));
	HAS_MASK(TXT("itemCollisionFlags"));

	HAS_MASK(TXT("actorCollidesWithFlags"));
	HAS_MASK(TXT("itemCollidesWithFlags"));

	HAS_MASK(TXT("ikTrace"));
	HAS_MASK(TXT("ikTraceReject"));
	
	HAS_MASK(TXT("presenceTrace"));
	HAS_MASK(TXT("presenceTraceReject"));

	HAS_MASK(TXT("playerCollidesWithFlags"));
	HAS_MASK(TXT("playerSlidingLocomotionCollidesWithFlags"));
...
{
	s_ikTrace = s_flags->flags.get_existing(Name(String(TXT("ikTrace"))));
	s_ikTraceReject = s_flags->flags.get_existing(Name(String(TXT("ikTraceReject"))));
	s_presenceTrace = s_flags->flags.get_existing(Name(String(TXT("presenceTrace"))));
	s_presenceTraceReject = s_flags->flags.get_existing(Name(String(TXT("presenceTraceReject"))));
	s_visionTrace = s_flags->flags.get_existing(Name(String(TXT("visionTrace"))));

	// update default (all - rejects)
	{
		Flags::s_default = Flags::s_all;
		if (s_ikTraceReject) Flags::s_default = Flags::s_default & -s_ikTraceReject->flags;
		if (s_presenceTraceReject) Flags::s_default = Flags::s_default & -s_presenceTraceReject->flags;
		for_every(flag, s_flags->flags)
		{
			if (String::does_contain_icase(flag->name.to_string().to_char(), TXT("reject")))
			{
				Flags::s_default = Flags::s_default & -flag->flags;
			}
		}
	}
