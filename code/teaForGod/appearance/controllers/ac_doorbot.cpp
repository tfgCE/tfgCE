...
// ai messages
DEFINE_STATIC_NAME_STR(aimSpeak, TXT("doorbot; speak"));
DEFINE_STATIC_NAME_STR(aimSpeakNo, TXT("doorbot; speak no"));
DEFINE_STATIC_NAME_STR(aimSpeakYes, TXT("doorbot; speak yes"));
...
Doorbot::Doorbot(DoorbotData const * _data)
: base(_data)
{
...
Doorbot::~Doorbot()
{
	if (auto* o = get_owner())
	{
		if (auto* imo = o->get_owner())
		{
			if (auto* ai = imo->get_ai())
			{
				if (auto* m = ai->get_mind())
				{
					m->access_execution().unregister_message_handler(this);
				}
			}
		}
	}
...
void Doorbot::initialise(Framework::ModuleAppearance* _owner)
{
	...
	if (auto* o = get_owner())
	{
		if (auto* imo = o->get_owner())
		{
			if (auto* ai = imo->get_ai())
			{
				if (auto* m = ai->get_mind())
				{
					m->access_execution().register_message_handler(NAME(aimSpeak), this);
					m->access_execution().register_message_handler(NAME(aimSpeakNo), this);
					m->access_execution().register_message_handler(NAME(aimSpeakYes), this);
				}
			}
		}
	}
...
void Doorbot::advance_and_adjust_preliminary_pose(REF_ Framework::AppearanceControllerPoseContext & _context)
{
	base::advance_and_adjust_preliminary_pose(_context);
}

void Doorbot::handle_message(Framework::AI::Message const& _message)
{
	if (_message.get_name() == NAME(aimSpeakNo))
	{
		speakNoRequests = 1;
		speakYesRequests = 0;
		speakRequestsTime = 0.0f;
	}
	if (_message.get_name() == NAME(aimSpeakYes))
	{
		speakNoRequests = 0;
		speakYesRequests = 1;
		speakRequestsTime = 0.0f;
	}
	if (_message.get_name() == NAME(aimSpeak))
	{
		speakNoRequests = 0;
		speakYesRequests = 0;
		speakRequestsTime = 0.1f;
		if (auto* p = _message.get_param(NAME(time)))
		{
			speakRequestsTime = max(speakRequestsTime, p->get_as<float>());
		}
	}
}

void Doorbot::calculate_final_pose(REF_ Framework::AppearanceControllerPoseContext & _context)
{
	base::calculate_final_pose(_context);

#ifdef MEASURE_PERFORMANCE_APPEARANCE_CONTROLLERS_DETAILED
	MEASURE_PERFORMANCE(doorbot_finalPose);
#endif
	...
	// head last
	{
		Quat useHeadLook = Quat::identity;

		{
			if (speakYesRequests > 0)
			{
			...
			speakRequestsTime = max(0.0f, speakRequestsTime - deltaTime);
			speakRequestsBlock = max(0.0f, speakRequestsBlock - deltaTime);
			if (speakRequestsTime > 0.0f)
			{
				if (speakRequestsBlock == 0.0f)
				{
					speakRequestsBlock = rg.get_float(0.2f, 0.4f);
					speakTime = 0.0f;
					Rotator3 sa;
		...
		headBoneMS.set_orientation(headBoneMS.get_orientation().to_world(useHeadLook));
...
void Doorbot::get_info_for_auto_processing_order(OUT_ ArrayStack<int> & dependsOnBones, OUT_ ArrayStack<int> & dependsOnParentBones, OUT_ ArrayStack<int> & usesBones, OUT_ ArrayStack<int> & providesBones,
...
REGISTER_FOR_FAST_CAST(DoorbotData);

Framework::AppearanceControllerData* DoorbotData::create_data()
{
	return new DoorbotData();
}

void DoorbotData::register_itself()
{
	Framework::AppearanceControllers::register_appearance_controller(NAME(doorbot), create_data);
}

DoorbotData::DoorbotData()
{
}

bool DoorbotData::load_from_xml(IO::XML::Node const * _node, Framework::LibraryLoadingContext & _lc)
{
...
bool DoorbotData::prepare_for_game(Framework::Library* _library, Framework::LibraryPrepareContext& _pfgContext)
{
...
Framework::AppearanceControllerData* DoorbotData::create_copy() const
{
	DoorbotData* copy = new DoorbotData();
	*copy = *this;
	return copy;
}

Framework::AppearanceController* DoorbotData::create_controller()
{
	return new Doorbot(this);
}

void DoorbotData::reskin(Meshes::BoneRenameFunc _rename)
{
	base::reskin(_rename);
	// no reskinning here
}

void DoorbotData::apply_mesh_gen_params(Framework::MeshGeneration::GenerationContext const & _context)
{
