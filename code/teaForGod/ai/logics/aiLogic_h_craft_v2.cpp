...
DEFINE_STATIC_NAME_STR(aim_circlePOI, TXT("h_craft; circle POI"));
DEFINE_STATIC_NAME_STR(aim_instantTransitSpeed, TXT("h_craft; instant transit speed"));
...
DEFINE_STATIC_NAME(followPOIsTowardsDistance);
DEFINE_STATIC_NAME(followPOIsRelativeDistance);
DEFINE_STATIC_NAME(circlePOI);
DEFINE_STATIC_NAME(circlePOIPitch);
DEFINE_STATIC_NAME(circlePOIRadiusRange);
DEFINE_STATIC_NAME(circlePOIClockwise);
DEFINE_STATIC_NAME(circlePOITowardsDistance);
DEFINE_STATIC_NAME(limitFollowRelVelocity);
DEFINE_STATIC_NAME(followRelVelocity);
DEFINE_STATIC_NAME(followRelVelocityDistance);
...
DEFINE_STATIC_NAME(blockMovementProcessingTime);
...
REGISTER_FOR_FAST_CAST(H_Craft_V2);

H_Craft_V2::H_Craft_V2(::Framework::AI::MindInstance* _mind, ::Framework::AI::LogicData const * _logicData)
: base(_mind, _logicData, execute_logic)
{
	h_craftData = fast_cast<H_Craft_V2Data>(_logicData);
...
H_Craft_V2::~H_Craft_V2()
{
	stop_observing_presence();
}

void H_Craft_V2::learn_from(SimpleVariableStorage& _parameters)
{
	base::learn_from(_parameters);

	blockMovementProcessingTime = _parameters.get_value<float>(NAME(blockMovementProcessingTime), blockMovementProcessingTime);
}

Transform H_Craft_V2::get_placement() const
{
...
void H_Craft_V2::on_presence_changed_room(Framework::ModulePresence* _presence, Framework::Room* _intoRoom, Transform const& _intoRoomTransform, Framework::DoorInRoom* _exitThrough, Framework::DoorInRoom* _enterThrough, Framework::DoorInRoomArrayPtr const* _throughDoors)
{
	if (location.is_set())
	{
		location = _intoRoomTransform.location_to_local(location.get());
	}
	if (rotation.is_set())
	{
		rotation = _intoRoomTransform.to_local(rotation.get().to_quat()).to_rotator();
	}
	velocityLinear = _intoRoomTransform.vector_to_local(velocityLinear);
	// velocityRotation is relative;
}

void H_Craft_V2::on_presence_destroy(Framework::ModulePresence* _presence)
{
	an_assert(observingPresence == _presence);
	stop_observing_presence();
}

void H_Craft_V2::observe_presence(Framework::ModulePresence* _presence)
{
...
void H_Craft_V2::stop_observing_presence()
{
...
void H_Craft_V2::advance(float _deltaTime)
{
	...
	bool prevProcessAsUsual = false;
	processAsUsual = false;
	...
	if (imo)
	{
		...
		if (blockMovementProcessingTime > 0.0f)
		{
			blockMovementProcessingTime -= _deltaTime;
			processAsUsual = false;
		}
	}
	...
	if (imo && processAsUsual)
	{
		if (auto* p = imo->get_presence())
		{
			if (!prevProcessAsUsual && processAsUsual)
			{
				velocityLinear = imo->get_presence()->get_velocity_linear();
				velocityRotation = imo->get_presence()->get_velocity_rotation();
			}
		...
		bool ignoreLimitFollowRelVelocity = false;
		...
		if (! craftOff && ! dying)
		{
			...
			if (followIMO.get())
			{
				...
				if (targetLocation.is_set() && location.is_set())
				{
					float dist = (targetLocation.get() - location.get()).length();
					if (dist > setup.transit.linSpeed)
					{
						nearApproach = false;
					}
					else
					{
						Vector3 velDiff = followInTransitVelocity.get() - velocityLinear;
						if (dist > velDiff.length() + setup.transit.linSpeed * 0.2f)
						{
							nearApproach = false;
						}
					}
					Vector3 atTargetDir = (targetLocation.get() - imo->get_presence()->get_placement().get_translation()).normal();
					float speedTarget = Vector3::dot(atTargetDir, followInTransitVelocity.get());
					float speedCurrent = Vector3::dot(atTargetDir, velocityLinear);
					if (speedCurrent > speedTarget)
					{
						float timeToSlowDown = (speedCurrent - speedTarget) / setup.transit.linAcceleration;
						timeToSlowDown -= 0.5f;
						float distSlowDown = speedCurrent * timeToSlowDown - setup.transit.linAcceleration * sqr(timeToSlowDown) * 0.5f;
						float willBeAtDist = dist + speedTarget * timeToSlowDown;
						if (distSlowDown > willBeAtDist)
						{
							// we're moving so that we should already start moving into the position
							nearApproach = true;
						}
					}
				}
				...
				{
					{
						bool movingAtTarget = false;
						if (!movingAtTarget &&
							followRelVelocity.is_set() &&
							...
							!instantFollowing) // ignore for instantFollowing
						{
							...
							if (!followRelVelocityDistance.is_set() || toTarget.length() < followRelVelocityDistance.get())
							{
				...
				{
					ignoreLimitFollowRelVelocity = true;
					if (targetLocation.is_set() && location.is_set())
					{
						forceFollowInTransit = true;
						followInTransitVelocity = (targetLocation.get() - location.get()).normal();
						{
							float speedReq = setup.transit.linSpeed;
							speedReq *= requestedTransitSpeedPt.get(1.0f);
							speedReq = requestedTransitSpeed.get(speedReq);
							followInTransitVelocity = followInTransitVelocity.get() * speedReq;
						}
						targetYaw = followInTransitVelocity.get().to_rotator().yaw;
					}
				}
...
#endif
							if (followPOIsTowardsDistance.is_set())
							{
								targetLocation = pAt + (towardsWS - pAt).normal() * followPOIsTowardsDistance.get();
							}
							else
							{
			...
			if (circlePOI.is_set())
			{
				...
				if (!circlePOIContext.poi.get() ||
					circlePOIContext.forPOI != circlePOI.get() ||
					circlePOIContext.forRoom != imo->get_presence()->get_in_room())
				{
					Framework::PointOfInterestInstance* found = nullptr;
					imo->get_presence()->get_in_room()->find_any_point_of_interest(circlePOI.get(), found);
					circlePOIContext.poi = found;
					circlePOIContext.forPOI = circlePOI.get();
					circlePOIContext.forRoom = imo->get_presence()->get_in_room();
				}
				if (auto* poi = circlePOIContext.poi.get())
				{
					Transform poiAt = poi->calculate_placement();

					{
						{
							Vector3 awayDirWS = (pAt - poiAt.get_translation());
							if (circlePOIPitch.is_set())
							{
								awayDirWS.z = 0.0f;
							}
							awayDirWS.normalise();
							Vector3 towardsDirWS = awayDirWS.rotated_by_yaw(circlePOIClockwise ? 90.0f : -90.0f);

							float speed = velocityLinear.length();

							{
								float dist = (poiAt.get_translation() - pAt).length_2d();
								{
									// try to stay at the circle
									float timeToCover = 0.7f;
									float angleCovered = ((speed * timeToCover) / (dist * 2.0f * pi<float>())) * 360.0f;
									angleCovered = clamp(angleCovered, 0.0f, 50.0f);
									towardsDirWS = towardsDirWS.rotated_by_yaw(circlePOIClockwise ? angleCovered : -angleCovered);
								}
								if (circlePOIRadiusRange.is_set())
								{
									// turn to be in the range
									if (dist < circlePOIRadiusRange.get().min)
									{
										towardsDirWS += awayDirWS * clamp((circlePOIRadiusRange.get().min - dist) / 80.0f, 0.0f, 0.3f);
										towardsDirWS.normalise();
									}
									else if (dist > circlePOIRadiusRange.get().max)
									{
										towardsDirWS -= awayDirWS * clamp((dist - circlePOIRadiusRange.get().max) / 80.0f, 0.0f, 0.3f);
										towardsDirWS.normalise();
									}
								}
							}
							...
							if (circlePOIPitch.is_set())
							{
								Rotator3 towardsRotWS = towardsDirWS.to_rotator();
								towardsRotWS.pitch = circlePOIPitch.get();
								towardsDirWS = towardsRotWS.get_forward();
							}

							Vector3 towardsWS = pAt + towardsDirWS * circlePOITowardsDistance.get(speed);
		...
		bool toBeInTransit = false;
		{
			...
			else
			{
				...
				else
				{
					...
					float yawDiff = Rotator3::normalise_axis(yawToTarget - rotation.get().yaw);
					if (distance2d > setup.changeToTransitDistance || forceFollowInTransit || transitThrough)
					{
						toBeInTransit = true;
						if (abs(yawDiff) < 3.0f || forceFollowInTransit)
						{
		...
		// current mode
		H_Craft_V2MovementParams const& use = inTransit ? setup.transit : setup.maneuver;
		...
		else
		{
			...
			if (targetLocation.is_set())
			{
				...
				if (followInTransitVelocity.is_set() && followInTransitOffsetOS.is_set())
				{
					Vector3 targetVelocity = followInTransitVelocity.get();
					{
						...
						targetVelocity = targetVelocity.normal() * clamp(targetVelocity.length(), targetSpeed * 0.95f, targetSpeed * 1.05f);
					}

					if (limitFollowRelVelocity.is_set() && ! ignoreLimitFollowRelVelocity)
					{
					...
					{
						if (targetVelocity.length_2d() * 1.4f < velocityLinear.length_2d())
						{
							Vector3 actTV = velocityLinear.normal() * targetVelocity.length();
							Vector3 changeRequiredSlowDown;
							changeRequiredSlowDown = actTV - velocityLinear;

							// first try to slow down
							changeRequired = lerp(0.8f, changeRequired, changeRequiredSlowDown);
						}
					}
			...
			// smooth out limit laterl speed
			{
				auto const& limitLateralSpeed = inTransit ? setup.transit.limitLateralSpeed : setup.maneuver.limitLateralSpeed;
				Optional<float> targetLLS = limitLateralSpeed;
				if (limitLateralSpeed.is_set())
				{
					if (!setup.limitLateralSpeed.is_set())
					{
						setup.limitLateralSpeed = max(100.0f, velocityLinear.length());
					}
				}
				else if (setup.limitLateralSpeed.is_set())
				{
					targetLLS = max(1000.0f, velocityLinear.length() * 10.0f);
				}
				if (setup.limitLateralSpeed.is_set() && targetLLS.is_set())
				{
					// smooth the limit
					setup.limitLateralSpeed = blend_to_using_time(setup.limitLateralSpeed.get(), targetLLS.get(), 5.0f, _deltaTime);
				}
			}

			// apply lateral speed limit, and still blend it
			{
				auto const& limitLateralSpeed = setup.limitLateralSpeed;
				if (limitLateralSpeed.is_set())
				{
					auto p = imo->get_presence()->get_placement();

					Vector3 velocityLinearOS = p.vector_to_local(velocityLinear);
					Vector3 lateral = velocityLinearOS * Vector3::xz;
					float lateraLength = lateral.length();
					if (lateraLength > limitLateralSpeed.get())
					{
						float totalSpeed = velocityLinearOS.length();
						float ySpeed = sqrt(max(0.0f, sqr(totalSpeed) - sqr(lateraLength)));
						ySpeed *= sign(velocityLinearOS.y);
						velocityLinearOS = lateral.normal() * limitLateralSpeed.get();
						velocityLinearOS.y = ySpeed;
						velocityLinear = lerp(clamp(_deltaTime / 0.3f, 0.0f, 1.0f), velocityLinear, p.vector_to_world(velocityLinearOS));
					}
				}
			}
		}
		...
		if (_deltaTime != 0.0f)
		{
			...
			if (use.levelRollBlendTime > 0.0f)
			{
				velocityRotation.roll = blend_to_using_time(velocityRotation.roll, -rotation.get().roll * 2.0f, use.levelRollBlendTime, _deltaTime);
			}
			if (use.levelPitchBlendTime > 0.0f)
			{
				velocityRotation.pitch = blend_to_using_time(velocityRotation.pitch, -rotation.get().pitch * 2.0f, use.levelPitchBlendTime, _deltaTime);
			}
		...
		debug_context(imo->get_presence()->get_in_room());
		{
			...
			debug_draw_arrow(true, Colour::mint, at, at + velocityLinear);
			{
				Transform p = imo->get_presence()->get_placement();
				Vector3 velOS = p.vector_to_local(velocityLinear);
				debug_push_transform(p);
				debug_draw_arrow(true, Colour::red, Vector3::zero, Vector3::xAxis * velOS.x);
				debug_draw_arrow(true, Colour::green, Vector3::zero, Vector3::yAxis * velOS.y);
				debug_draw_arrow(true, Colour::blue, Vector3::zero, Vector3::zAxis * velOS.z);
				debug_pop_transform();
			}
		}
		debug_no_context();
...
void H_Craft_V2::reset_requests(bool _keepFollowOffsetAdditional)
{
	devTeleport = false;
	...
	followPOIs.clear();
	followPOIsTowardsDistance.clear();
	followPOIsRelativeDistance.clear();
	circlePOI.clear();
	circlePOIPitch.clear();
	circlePOIRadiusRange.clear();
	circlePOIClockwise = true;
	circlePOITowardsDistance.clear();
	limitFollowRelVelocity.clear();
	followRelVelocity.clear();
	followRelVelocityDistance.clear();
	followOffset.clear();
...
LATENT_FUNCTION(H_Craft_V2::execute_logic)
{
	LATENT_SCOPE();
	...
	auto* self = fast_cast<H_Craft_V2>(logic);
	...
	messageHandler.use_with(mind);
	{
		...
		messageHandler.set(NAME(aim_follow), [self](Framework::AI::Message const& _message)
		{
			...
			READ_PARAM(float, followRelVelocityDistance);
		...
		messageHandler.set(NAME(aim_followPOIs), [self](Framework::AI::Message const& _message)
		{
			...
			if (auto* followPOIsTowardsDistancePtr = _message.get_param(NAME(followPOIsTowardsDistance)))
			{
				self->followPOIsTowardsDistance = followPOIsTowardsDistancePtr->get_as<float>();
			}
			if (auto* followPOIsRelativeDistancePtr = _message.get_param(NAME(followPOIsRelativeDistance)))
			{
				self->followPOIsRelativeDistance = followPOIsRelativeDistancePtr->get_as<float>();
			}
		...
		messageHandler.set(NAME(aim_circlePOI), [self](Framework::AI::Message const& _message)
		{
			bool keepActiveRequest = false;
			if (auto * ptr = _message.get_param(NAME(keepActiveRequest)))
			{
			...
			if (auto* circlePOIPtr = _message.get_param(NAME(circlePOI)))
			{
				self->circlePOI = circlePOIPtr->get_as<Name>();
			}
			if (auto* circlePOIPitchPtr = _message.get_param(NAME(circlePOIPitch)))
			{
				self->circlePOIPitch = circlePOIPitchPtr->get_as<float>();
			}
			if (auto* circlePOIRadiusRangePtr = _message.get_param(NAME(circlePOIRadiusRange)))
			{
				self->circlePOIRadiusRange = circlePOIRadiusRangePtr->get_as<Range>();
			}
			if (auto* circlePOIClockwisePtr = _message.get_param(NAME(circlePOIClockwise)))
			{
				self->circlePOIClockwise = circlePOIClockwisePtr->get_as<bool>();
			}
			if (auto* circlePOITowardsDistancePtr = _message.get_param(NAME(circlePOITowardsDistance)))
			{
				self->circlePOITowardsDistance = circlePOITowardsDistancePtr->get_as<float>();
			}

			self->transitThrough = true; // by default
			READ_PARAM(float, requestedManeuverSpeed);
			READ_PARAM(float, requestedManeuverSpeedPt);
			READ_PARAM(float, requestedManeuverAccelPt);
			READ_PARAM(float, requestedTransitSpeed);
		...
		messageHandler.set(NAME(aim_instantTransitSpeed), [self](Framework::AI::Message const& _message)
		{
			self->instantTransitSpeed = 1.0f; // if no params, but let override
			if (auto* instantTransitSpeedPtr = _message.get_param(NAME(instantTransitSpeed)))
			{
				if (instantTransitSpeedPtr->get_type() == type_id<bool>())
				{
...
Vector3 H_Craft_V2::FollowPOIPath::process(FoundPOI const* _pois, int _poiCount, float _relativeDistance)
{
...
REGISTER_FOR_FAST_CAST(H_Craft_V2Data);

bool H_Craft_V2Data::load_from_xml(IO::XML::Node const* _node, ::Framework::LibraryLoadingContext& _lc)
{
...
bool H_Craft_V2Data::prepare_for_game(::Framework::Library* _library, ::Framework::LibraryPrepareContext& _pfgContext)
{
...
bool H_Craft_V2MovementParams::load_from_xml(IO::XML::Node const* _node)
{
	bool result = true;
	...
	{
		levelRollBlendTime = node->get_float_attribute(TXT("levelRollBlendTime"), levelRollBlendTime);
		levelPitchBlendTime = node->get_float_attribute(TXT("levelPitchBlendTime"), levelPitchBlendTime);
	...
	limitLateralSpeed.load_from_xml(_node, TXT("limitLateralSpeed"));
...
bool H_Craft_V2Setup::load_from_xml(IO::XML::Node const* _node)
{
	bool result = true;
