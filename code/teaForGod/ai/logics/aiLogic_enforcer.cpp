...
REGISTER_FOR_FAST_CAST(Enforcer);

Enforcer::Enforcer(::Framework::AI::MindInstance* _mind, ::Framework::AI::LogicData const * _logicData)
...
void Enforcer::learn_from(SimpleVariableStorage& _parameters)
{
...
LATENT_FUNCTION(Enforcer::attack_enemy)
{
	LATENT_SCOPE();
	...
	auto* self = fast_cast<Enforcer>(mind->get_logic());
	an_assert(npcBase);
	...
	SHOOT:
		if (self->get_max_attack_distance().is_set())
		{
			if (enemyPlacement.calculate_string_pulled_distance(imo->get_presence()->get_centre_of_presence_os(), Transform(enemyTargetingOffsetOS, Quat::identity)) > self->get_max_attack_distance().get())
			{
				goto POSTSHOT;
			}
		}
						   ...
						   enemyPlacement.get_in_final_room(), enemyPlacement.get_placement_in_owner_room()))
		{
			...
			LATENT_WAIT(0.2f * GameSettings::get().difficulty.aiReactionTime);
			postShootRequired = false;
...
LATENT_FUNCTION(Enforcer::execute_logic)
{
	LATENT_SCOPE();
	...
	LATENT_VAR(SimpleVariableInfo, headlessVar);
	LATENT_VAR(::Framework::AI::LatentTaskHandle, headlessDischargeTask);
	...
	while (true)
	{
		if (!headlessDischargeTask.is_running() &&
			...
			if (auto* imo = mind->get_owner_as_modules_owner())
			{
				if (auto* h = imo->get_custom<CustomModules::Health>())
				{
					if (h->is_alive())
					{
						...
						nextTask.propose(AI_LATENT_TASK_FUNCTION(Tasks::lightning_random_discharge));
						headlessDischargeTask.start_latent_task(mind, nextTask);
					}
				}
			}
		}
