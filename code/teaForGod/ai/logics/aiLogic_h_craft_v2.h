...
namespace TeaForGodEmperor
{
	...
	namespace AI
	{
		namespace Logics
		{
			class H_Craft_V2Data;

			struct H_Craft_V2MovementParams
			{
				...
				// besides offsets
				float levelRollBlendTime = 0.5f;
				float levelPitchBlendTime = 0.5f;

				// offsets do not affect our actual placement, just the one we see from the outside
				float velocityYawToRoll = 0.2f;
				float yAccelToPitch = -2.0f;
				float xAccelToRoll = 2.5f;

				Optional<float> limitLateralSpeed; // if set, will keep speed going forward/backward but limit lateral movement, this is a hard limit and will be blended to give a smooth result

				bool load_from_xml(IO::XML::Node const* _node);
			};

			struct H_Craft_V2Setup
			{
				...
				H_Craft_V2MovementParams maneuver;
				H_Craft_V2MovementParams transit;

				Optional<float> limitLateralSpeed;
			...
			{
				FAST_CAST_DECLARE(H_Craft_V2);
				FAST_CAST_BASE(::Framework::AI::LogicWithLatentTask);
				FAST_CAST_END();

				typedef ::Framework::AI::LogicWithLatentTask base;
			public:
				static ::Framework::AI::Logic* create_ai_logic(::Framework::AI::MindInstance* _mind, ::Framework::AI::LogicData const * _logicData) { return new H_Craft_V2(_mind, _logicData); }

			public:
				H_Craft_V2(::Framework::AI::MindInstance* _mind, ::Framework::AI::LogicData const * _logicData);
				virtual ~H_Craft_V2();
			...
			protected:
				H_Craft_V2Data const * h_craftData = nullptr;

				Random::Generator rg;

				// setup
				H_Craft_V2Setup setup;
				...
				bool processAsUsual = false;
				float blockMovementProcessingTime = 0.0f;
				...
				Optional<float> followRelVelocityDistance; // only when we're at certain distance
				...
				Optional<float> followPOIsTowardsDistance; // pretend that the location we're moving to is further, this helps with getting higher speed
				Optional<float> followPOIsRelativeDistance; // relative on path to find POI to move towards
				Optional<Name> circlePOI;
				Optional<float> circlePOIPitch;
				Optional<Range> circlePOIRadiusRange;
				bool circlePOIClockwise = true;
				Optional<float> circlePOITowardsDistance; // as for follow POIs
				...
				struct CirclePOIContext
				{
					Name forPOI;
					Framework::Room* forRoom = nullptr; // unsafe ref
			...
			{
				FAST_CAST_DECLARE(H_Craft_V2Data);
				FAST_CAST_BASE(::Framework::AI::LogicData);
				FAST_CAST_END();

				typedef ::Framework::AI::LogicData base;
			public:
				static ::Framework::AI::LogicData* create_ai_logic_data() { return new H_Craft_V2Data(); }
