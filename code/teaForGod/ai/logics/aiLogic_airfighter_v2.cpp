...
DEFINE_STATIC_NAME(attackInterval);
...
REGISTER_FOR_FAST_CAST(Airfighter_V2);
...
Airfighter_V2::Airfighter_V2(::Framework::AI::MindInstance* _mind, ::Framework::AI::LogicData const * _logicData)
: base(_mind, _logicData)
{
	airfighterData = fast_cast<Airfighter_V2Data>(_logicData);
		...
		messageHandler.set(NAME(aim_addTarget), [this](Framework::AI::Message const& _message)
			{
				...
				if (auto* ptr = _message.get_param(NAME(targetIMO)))
				{
					...
					{
						if (atiPtr->get_type() == type_id<Range>())
						{
							ti.attackLength = atiPtr->get_as<Range>();
						}
						else
						{
							ti.attackLength = Range(atiPtr->get_as<float>());
						}
					}
					if (auto* atiPtr = _message.get_param(NAME(attackInterval)))
					{
						if (atiPtr->get_type() == type_id<Range>())
						{
							ti.attackInterval = atiPtr->get_as<Range>();
						}
						else
						{
							ti.attackInterval = Range(atiPtr->get_as<float>());
						}
					}
...
Airfighter_V2::~Airfighter_V2()
{
}

void Airfighter_V2::advance(float _deltaTime)
{
	if (processAsUsual && shooterWaveActive)
	{
	...
	if (processAsUsual)
	{
		attackWaitTimeLeft = max(0.0f, attackWaitTimeLeft - _deltaTime);
	...
	if (processAsUsual && (beActive || beActiveAttack))
	{
		an_assert(!shooterWaveActive);
		if (attack)
		{
			if (currentAttackLength == 0.0f)
			{
				currentAttackLength = Random::get(targetList.get_first().attackLength.get(Range::zero));
			}
			if (currentTarget.is_set() && !targetList.is_empty() &&
				...
				attackTime >= currentAttackLength)
			{
				if (targetList.get_first().attackInterval.is_set())
				{
					attackWaitTimeLeft = Random::get(targetList.get_first().attackInterval.get());
					currentAttackLength = 0.0f; // reset
				}
				else
				{
					currentTarget.clear();
		...
		if (currentTarget.get())
		{
			if (auto* imo = get_mind()->get_owner_as_modules_owner())
			{
				if (imo->get_presence()->get_in_room() == currentTarget->get_presence()->get_in_room())
				{
					...
					if (beActive)
					{
						Optional<State> setupState;

						if (airfighterSetup.update)
						{
							airfighterSetup.update = false;
							setupState = state;
						}

						if (airfighterSetup.flyByAttack)
						{
							if (state == State::None || state == State::FlyByApproach )
							{
								setupState = State::FlyByApproach;
							}
							if (state == State::FlyByApproach)
							{
								if (distance < airfighterSetup.flyByAttackDistance.get(60.0f))
								{
									setupState = State::FlyByAttack;
								}
							}
						...
						if (setupState.is_set())
						{
							...
							if (state == State::FlyByApproach)
							{
								...
								followRelVelocityDistance = airfighterSetup.flyByAttackDistance.get(60.0f) * 1.6f;
...
REGISTER_FOR_FAST_CAST(Airfighter_V2Data);

bool Airfighter_V2Data::load_from_xml(IO::XML::Node const* _node, ::Framework::LibraryLoadingContext& _lc)
{
...
bool Airfighter_V2Data::prepare_for_game(::Framework::Library* _library, ::Framework::LibraryPrepareContext& _pfgContext)
{
