#include "aiLogicTask_announcePresence.h"
...
bool TeaForGodEmperor::AI::Logics::ShouldTask::announce_presence(Framework::IModulesOwner* imo)
{
	if (imo && GameSettings::get().difficulty.allowAnnouncePresence > 0.0f)
	{
...
LATENT_FUNCTION(TeaForGodEmperor::AI::Logics::Tasks::announce_presence)
{
	LATENT_SCOPE();
			...
			if (auto* imo = mind->get_owner_as_modules_owner())
			{
				if (auto* p = imo->get_presence())
				{
					if (auto* r = p->get_in_room())
					{
						...
						{
							if (Random::get_chance(GameSettings::get().difficulty.allowAnnouncePresence))
							{
								if (auto* s = imo->get_sound())
								{
