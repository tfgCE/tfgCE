...
DEFINE_STATIC_NAME(noHeadShoot);
DEFINE_STATIC_NAME(noHeadMuzzleFlash);

//

REGISTER_FOR_FAST_CAST(Trigger);

Trigger::Trigger(::Framework::AI::MindInstance* _mind, ::Framework::AI::LogicData const * _logicData)
...
void Trigger::learn_from(SimpleVariableStorage& _parameters)
{
...
LATENT_FUNCTION(Trigger::attack_enemy)
{
	LATENT_SCOPE();
	...
	auto* self = fast_cast<Trigger>(mind->get_logic());
	an_assert(npcBase);
...
static LATENT_FUNCTION(headless)
{
	LATENT_SCOPE();
	LATENT_SCOPE_INFO(TXT("headless"));

	LATENT_PARAM(::Framework::AI::MindInstance*, mind); // auto
	LATENT_PARAM_NOT_USED(::Framework::AI::LatentTaskHandle*, thisTaskHandle); // auto
	LATENT_END_PARAMS();

	LATENT_VAR(int, shootIdx);
	...
	ai_log_colour(mind->get_logic(), Colour::blue);
	ai_log(mind->get_logic(), TXT("headless"));
	ai_log_no_colour(mind->get_logic());
	...
	{
		{
			for (int i = 0; i < 1 || shootIdx < 3; ++i, ++shootIdx)
			{
				if (auto* tos = imo->get_temporary_objects())
				{
					auto* projectile = tos->spawn_in_room(NAME(noHeadShoot), Framework::ModuleTemporaryObjects::SpawnParams().at_socket(NAME(headshot)));
					if (projectile)
					{
						...
						todo_note(TXT("hardcoded, it is headless, just go randomly"));
						{
							Vector3 velocity = Vector3(0.0f, 10.0f, 0.0f);
							Quat atAngle = Rotator3(Random::get_float(10.0f, 30.0f), Random::get_float(-180.0f, 180.0f), 0.0f).to_quat();
							Quat velocityDir = velocity.to_rotator().to_quat();
							velocity = velocityDir.to_world(atAngle).get_y_axis() * velocity.length();
							projectile->on_activate_set_relative_velocity(velocity);
						}
					}
					//tos->spawn_attached(NAME(noHeadMuzzleFlash), NAME(headshot));
				}
				if (auto* s = imo->get_sound())
				{
					s->play_sound(NAME(noHeadShoot), NAME(headshot));
				}
			}
		}
		LATENT_WAIT(Random::get_float(0.4f, 1.0f));
	}
	// ...
...
LATENT_FUNCTION(Trigger::execute_logic)
{
	LATENT_SCOPE();
	...
	LATENT_VAR(SimpleVariableInfo, headlessVar);
	LATENT_VAR(::Framework::AI::LatentTaskHandle, headlessTask);
	...
	while (true)
	{
		if (!headlessTask.is_running() &&
			headlessVar.get<bool>())
		{
			if (auto* imo = mind->get_owner_as_modules_owner())
			{
				if (auto* h = imo->get_custom<CustomModules::Health>())
				{
					if (h->is_alive())
					{
						...
						nextTask.propose(AI_LATENT_TASK_FUNCTION(headless));
						headlessTask.start_latent_task(mind, nextTask);
					}
				}
			}
		}
