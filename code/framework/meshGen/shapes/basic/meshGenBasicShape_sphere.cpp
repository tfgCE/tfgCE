...
REGISTER_FOR_FAST_CAST(SphereData);

bool SphereData::load_from_xml(IO::XML::Node const * _node, LibraryLoadingContext & _lc)
{
...
ElementShapeData* BasicShapes::create_sphere_data()
{
...
bool BasicShapes::sphere(GenerationContext & _context, ElementInstance & _instance, ::Framework::MeshGeneration::ElementShapeData const * _data)
{
	...
	if (SphereData const * sData = fast_cast<SphereData>(_data))
	{
		legacyGenerator = sData->legacyGenerator;
	}
	...
	if (legacyGenerator)
	{
		...
		ipu.will_need_at_least_points((hiMax + 1) * viMax);
		...
		for (int vi = 0; vi <= viMax; vi++)
		{
			...
			for (int hi = 0; hi <= hiMax; hi++)
			{
				...
				float hc = cos_deg(ha);
				float hs = sin_deg(ha);
				ipu.add_point(Vector3(hs * vc, hc * vc, vs) * useRadius);
			}
		}
		...
		for (int vi = 0; vi < viMax; vi++)
		{
			...
			for (int hi = 0; hi < hiMax; hi++)
			{
				...
				{
					ipu.add_quad(0.0f, upper, upper + 1, lower + 1, lower);
				}
				else
				{
	...
	else
	{
		...
		ipu.will_need_at_least_polygons(onePatchTriCount * 8);
				...
				for_count(int, u, uCount)
				{
					...
					for_count(int, v, vCount)
					{
						...
						if (lo)
						{
							if (doReverseSurfaces)
							{
								ipu.add_triangle(0.0f, loIdx, loIdx + 1, hiIdx);
							}
							else
							{
						...
						else
						{
							if (doReverseSurfaces)
							{
								ipu.add_triangle(0.0f, hiIdx, loIdx + 1, hiIdx + 1);
							}
							else
							{
	...
	if (_context.does_require_movement_collision() && useRadius > 0.0f)
	{
		if (auto const * data = fast_cast<Data>(_data))
		{
			if (data->createMovementCollision.create)
			{
				...
				sphere.setup(Vector3::zero, useRadius);
				sphere.use_material(data->createMovementCollision.get_physical_material(_context, _instance));
				part->add(sphere);
	...
	if (_context.does_require_precise_collision() && useRadius > 0.0f)
	{
		if (auto const * data = fast_cast<Data>(_data))
		{
			if (data->createPreciseCollision.create)
			{
				...
				Collision::Sphere sphere;
				sphere.setup(Vector3::zero, useRadius);
				sphere.use_material(data->createPreciseCollision.get_physical_material(_context, _instance));
				part->add(sphere);
	...
	if (!ipu.is_empty())
	{
		...
		part->debugInfo = String::printf(TXT("sphere @ %S"), _instance.get_element()->get_location_info().to_char());
#endif
