...
namespace Framework
{
	...
	{
		class ProvidePlacementFromBoneMSData;

		/**
		 *	
		 */
		class ProvidePlacementFromBoneMS
		: public AppearanceController
		{
			FAST_CAST_DECLARE(ProvidePlacementFromBoneMS);
			FAST_CAST_BASE(AppearanceController);
			FAST_CAST_END();

			typedef AppearanceController base;
		public:
			ProvidePlacementFromBoneMS(ProvidePlacementFromBoneMSData const * _data);
			virtual ~ProvidePlacementFromBoneMS();
		...
		protected:
			virtual tchar const * get_name_for_log() const { return TXT("provide placement from bone ms"); }

		private:
			ProvidePlacementFromBoneMSData const * providePlacementFromBoneMSData;
		...
		{
			FAST_CAST_DECLARE(ProvidePlacementFromBoneMSData);
		...
		public:
			...
			ProvidePlacementFromBoneMSData();
		...
		private:
			...
			MeshGenParam<Transform> offset = Transform::identity;

			static AppearanceControllerData* create_data();
				
			friend class ProvidePlacementFromBoneMS;
		};
	};
};
