...
namespace Framework
{
	...
	{
		class AnimTransformToVarData;

		/**
		 *	
		 */
		class AnimTransformToVar
		: public AppearanceController
		{
			FAST_CAST_DECLARE(AnimTransformToVar);
			FAST_CAST_BASE(AppearanceController);
			FAST_CAST_END();

			typedef AppearanceController base;
		public:
			AnimTransformToVar(AnimTransformToVarData const * _data);
			virtual ~AnimTransformToVar();
		...
		protected:
			virtual tchar const * get_name_for_log() const { return TXT("apply transform to var"); }

		private:
			AnimTransformToVarData const * animTransformToVarData;

			SimpleVariableInfo inVar;
			...
			Array<AnimTransform> animTransforms;
			int animTransformIdx = 0;
			int animTransformPrevIdx = 0;
			float animTransformTimeLeft = 0.0f;
		};

		class AnimTransformToVarData
		: public AppearanceControllerData
		{
			FAST_CAST_DECLARE(AnimTransformToVarData);
		...
		private:
			...
			struct AnimTransform 
			{
				MeshGenParam<Transform> animTransform;
				MeshGenParam<float> time = 0.5f;
			};
			Array<AnimTransform> animTransforms;

			static AppearanceControllerData* create_data();
				
			friend class AnimTransformToVar;
		};
	};
};
