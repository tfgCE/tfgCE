...
namespace Framework
{
	...
	{
		class ApplyBoneOffsetData;

		/**
		 *	
		 */
		class ApplyBoneOffset
		: public AppearanceController
		{
			FAST_CAST_DECLARE(ApplyBoneOffset);
			FAST_CAST_BASE(AppearanceController);
			FAST_CAST_END();

			typedef AppearanceController base;
		public:
			ApplyBoneOffset(ApplyBoneOffsetData const * _data);
			virtual ~ApplyBoneOffset();
		...
		protected:
			virtual tchar const * get_name_for_log() const { return TXT("apply bone offset"); }

		private:
			ApplyBoneOffsetData const * applyBoneOffsetData;

			SimpleVariableInfo useVar;
		...
		{
			FAST_CAST_DECLARE(ApplyBoneOffsetData);
