...
namespace Framework
{
	...
	{
		class ApplyTransformToVarData;

		/**
		 *	
		 */
		class ApplyTransformToVar
		: public AppearanceController
		{
			FAST_CAST_DECLARE(ApplyTransformToVar);
			FAST_CAST_BASE(AppearanceController);
			FAST_CAST_END();

			typedef AppearanceController base;
		public:
			ApplyTransformToVar(ApplyTransformToVarData const * _data);
			virtual ~ApplyTransformToVar();
		...
		private:
			ApplyTransformToVarData const * applyTransformToVarData;
		...
		{
			FAST_CAST_DECLARE(ApplyTransformToVarData);
		...
		private:
			...
			MeshGenParam<Transform> applyTransform;

			static AppearanceControllerData* create_data();
				
			friend class ApplyTransformToVar;
		};
	};
};
