...
namespace Framework
{
	...
	{
		class OrientAsOtherBoneData;

		/**
		*	We roll (Y axis stays same) to match orientation of other bone, we can choose which other axis we prefer X or Z
		*
		*/
		class OrientAsOtherBone
		: public AppearanceController
		{
			FAST_CAST_DECLARE(OrientAsOtherBone);
			FAST_CAST_BASE(AppearanceController);
			FAST_CAST_END();

			typedef AppearanceController base;
		public:
			OrientAsOtherBone(OrientAsOtherBoneData const * _data);
			virtual ~OrientAsOtherBone();
		...
		protected:
			virtual tchar const * get_name_for_log() const { return TXT("orient as other bone"); }

		private:
			OrientAsOtherBoneData const * orientAsOtherBoneData;
			Meshes::BoneID bone;
			Meshes::BoneID otherBone;
			Meshes::BoneID otherBoneB;
			float otherBoneBWeight;
			Meshes::BoneID boneParent;

			Optional<Quat> prevOrientateTo;
			float smoothOrientateToTime;
		};

		class OrientAsOtherBoneData
		: public AppearanceControllerData
		{
			FAST_CAST_DECLARE(OrientAsOtherBoneData);
		...
		private:
			...
			MeshGenParam<Name> otherBone;
			MeshGenParam<Name> otherBoneB;
			MeshGenParam<float> otherBoneBWeight;
			MeshGenParam<float> smoothOrientateToTime;

			static AppearanceControllerData* create_data();

			friend class OrientAsOtherBone;
		};
	};
};
